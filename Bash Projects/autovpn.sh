...
configure_firewall() {
    ...
    if [ "$commands" == "establish" ]; then
        expect -c "
            ...
            expect \"#\"
            send \"set network ike crypto-profiles ike-crypto-profiles NPT$team_num hash sha384 dh-group group20 encryption aes-256-cbc lifetime seconds 28800\\r\"
            expect \"#\"
            send \"set network ike crypto-profiles ipsec-crypto-profiles NPT$team_num esp authentication sha256 encryption aes-256-cbc\\r\"
            expect \"#\"
            send \"set network ike crypto-profiles ipsec-crypto-profiles NPT$team_num lifetime seconds 3600\\r\"
            expect \"#\"
            send \"set network ike crypto-profiles ipsec-crypto-profiles NPT$team_num dh-group group20\\r\"
            expect \"#\"
            send \"set network ike gateway NPT$team_num authentication pre-shared-key key $psk_key\\r\"
            expect \"#\"
            send \"set network ike gateway NPT$team_num protocol ikev2 dpd enable yes\\r\"
            expect \"#\"
            send \"set network ike gateway NPT$team_num protocol ikev2 ike-crypto-profile NPT$team_num\\r\"
            expect \"#\"
            send \"set network ike gateway NPT$team_num protocol version ikev2\\r\"
            expect \"#\"
            send \"set network ike gateway NPT$team_num local-address interface ethernet1/$int_num ip $wan_addr/28\\r\"
            expect \"#\"
            send \"set network ike gateway NPT$team_num protocol-common nat-traversal enable no\\r\"
            expect \"#\"
            send \"set network ike gateway NPT$team_num protocol-common fragmentation enable yes\\r\"
            expect \"#\"
            send \"set network ike gateway NPT$team_num peer-address ip $peer_addr\\r\"
            expect \"#\"
            send \"set network ike gateway NPT$team_num local-id id "$team_num"npt@nerd.home.lab type ufqdn\\r\"
            expect \"#\"
            send \"set network tunnel ipsec NPT$team_num auto-key ike-gateway NPT$team_num\\r\"
            expect \"#\"
            send \"set network tunnel ipsec NPT$team_num auto-key ipsec-crypto-profile NPT$team_num\\r\"
            expect \"#\"
            send \"set network tunnel ipsec NPT$team_num tunnel-monitor enable no\\r\"
            expect \"#\"
            send \"set network tunnel ipsec NPT$team_num tunnel-interface tunnel.$team_num\\r\"
            expect \"#\"
            send \"set network tunnel ipsec NPT$team_num anti-replay yes\\r\"
            expect \"#\"
            send \"set network tunnel ipsec NPT$team_num copy-tos yes\\r\"
            expect \"#\"
            send \"set network tunnel ipsec NPT$team_num disabled no\\r\"
            expect \"#\"
            send \"set network tunnel ipsec NPT$team_num tunnel-monitor destination-ip 192.168.$octet.1 enable yes tunnel-monitor-profile default\\r\"
            expect \"#\"
        ...
        expect -c "
            ...
            expect \"#\"
            send \"delete network tunnel ipsec NPT$team_num\\r\"
            expect \"#\"
            ...
            expect \"#\"
            send \"delete network ike gateway NPT$team_num\\r\"
            expect \"#\"
            send \"delete network ike crypto-profiles ipsec-crypto-profiles NPT$team_num\\r\"
            expect \"#\"
            send \"delete network ike crypto-profiles ike-crypto-profiles NPT$team_num\\r\"
            expect \"#\"
...
20 70 20 \
        ...
        "Team Number:"          "$team_num"     "Your NPT, team number; ie 401, 154, 03, 600" \
