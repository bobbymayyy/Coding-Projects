...
DEBIAN:
  ENABLE=${DEBIAN_ENABLE:-<unset>}
  URL=${DEBIAN_MIRROR_URL:-<unset>}
  SUITE=${DEBIAN_SUITE:-<unset>}
  COMPONENTS=${DEBIAN_COMPONENTS:-<unset>}
  PREFIX=${DEBIAN_PUBLISH_PREFIX:-<unset>}

UBUNTU:
  ENABLE=${UBUNTU_ENABLE:-<unset>}
  URL=${UBUNTU_MIRROR_URL:-<unset>}
  SUITE=${UBUNTU_SUITE:-<unset>}
  COMPONENTS=${UBUNTU_COMPONENTS:-<unset>}
  PREFIX=${UBUNTU_PUBLISH_PREFIX:-<unset>}

KALI:
  ENABLE=${KALI_ENABLE:-<unset>}
  URL=${KALI_MIRROR_URL:-<unset>}
  SUITE=${KALI_SUITE:-<unset>}
  COMPONENTS=${KALI_COMPONENTS:-<unset>}
  PREFIX=${KALI_PUBLISH_PREFIX:-<unset>}

PROXMOX:
  ENABLE=${PROXMOX_ENABLE:-<unset>}
  URL=${PROXMOX_MIRROR_URL:-<unset>}
  SUITE=${PROXMOX_SUITE:-<unset>}
  COMPONENTS=${PROXMOX_COMPONENTS:-<unset>}
  PREFIX=${PROXMOX_PUBLISH_PREFIX:-<unset>}

RPM:
  ENABLE_RPM=${ENABLE_RPM:-<unset>}
  RPM_REPO_DIR=${RPM_REPO_DIR:-<unset>}
  RPM_SOURCES:
$(printf '%s\n' "${RPM_SOURCES:-<none>}" | sed 's/^/    /')

ALPINE:
  ENABLE_ALPINE=${ENABLE_ALPINE:-<unset>}
  ALPINE_RSYNC_URL=${ALPINE_RSYNC_URL:-<unset>}
  ALPINE_BRANCHES=${ALPINE_BRANCHES:-<unset>}

Update script:
  ${UPDATE_SCRIPT:-<unset>}
EOF
  dialog --title "Current Config" --textbox "$tmp" 28 92
  rm -f "$tmp"
  ;;
