    ...
    try:
        ...
        if commands == "deploy":
            command_list = [
                ...
                f"set network ike crypto-profiles ike-crypto-profiles NPT{team_num} hash sha384 dh-group group20 encryption aes-256-cbc lifetime seconds 28800",
                f"set network ike crypto-profiles ipsec-crypto-profiles NPT{team_num} esp authentication sha256 encryption aes-256-cbc",
                f"set network ike crypto-profiles ipsec-crypto-profiles NPT{team_num} lifetime seconds 3600",
                f"set network ike crypto-profiles ipsec-crypto-profiles NPT{team_num} dh-group group20",
                f"set network ike gateway NPT{team_num} authentication pre-shared-key key {psk_key}",
                f"set network ike gateway NPT{team_num} protocol ikev2 dpd enable yes",
                f"set network ike gateway NPT{team_num} protocol ikev2 ike-crypto-profile NPT{team_num}",
                f"set network ike gateway NPT{team_num} protocol version ikev2",
                f"set network ike gateway NPT{team_num} local-address interface ethernet1/{int_num} ip {wan_addr}/28",
                f"set network ike gateway NPT{team_num} protocol-common nat-traversal enable no",
                f"set network ike gateway NPT{team_num} protocol-common fragmentation enable yes",
                f"set network ike gateway NPT{team_num} peer-address ip {peer_addr}",
                f"set network ike gateway NPT{team_num} local-id id {team_num}npt@nerd.home.lab type ufqdn",
                f"set network tunnel ipsec NPT{team_num} auto-key ike-gateway NPT{team_num}",
                f"set network tunnel ipsec NPT{team_num} auto-key ipsec-crypto-profile NPT{team_num}",
                f"set network tunnel ipsec NPT{team_num} tunnel-monitor enable no",
                f"set network tunnel ipsec NPT{team_num} tunnel-interface tunnel.{team_num}",
                f"set network tunnel ipsec NPT{team_num} anti-replay yes",
                f"set network tunnel ipsec NPT{team_num} copy-tos yes",
                f"set network tunnel ipsec NPT{team_num} disabled no",
                f"set network tunnel ipsec NPT{team_num} tunnel-monitor destination-ip 192.168.{octet}.1 enable yes tunnel-monitor-profile default",
            ...
            command_list = [
                ...
                f"delete network tunnel ipsec NPT{team_num}",
                ...
                f"delete network ike gateway NPT{team_num}",
                f"delete network ike crypto-profiles ipsec-crypto-profiles NPT{team_num}",
                f"delete network ike crypto-profiles ike-crypto-profiles NPT{team_num}",
